...
% \hlc[yellow]{  A stone is captured if all the adjacent intersections have opponent stones on them. It gets more complex when chains of stones of the same colour connected adjacently forms, these can only be captured if all stones in the chain have no liberties empty of the enemy stone. There are other rules concerning the legality of stone placements such as the Ko rules to prevent infinite loops in the game play where the players are in situation where same stones are placed and captured. Also, it is considered an illegal move to place a stone into a point which is surrounded by enemy stones at all adjacent points unless it will capture the enemy stones. The game ends when both players pass in succession and then starts counting the amount of territories held in control by their respectively stone strings. The aim of the game is to surround and capture as much territory possible and have more than the opponent.}
...
% \hlc[yellow]{The aim of this project was to create a program which allows users to play and solve Go life and death problems. While doing so, also develop an AI to play against the human player in a restricted life and death problem situation. Another aim of this project was to have two separate modes of use. An editor mode, which allows the user to create life and death problems and save them so that they could be played out later by other users. As well as a player mode which would allow the user to load up a Go problem and play it through versus the computer. The first stage of the project was to use brute force methods to solve simpler problems within a small-time frame. Later on, the main goal was to enhance the AI to play using heuristics so that it could solve larger problems in a feasible time period to be played versus a human. To finalize the project, the program was adjusted accordingly to restrict the scale of problems allowable and a set of problems were to be created in varying scale and difficulties for users to play through.}
...
To begin with the first milestone, aim of the project which is to create an AI to play against user in Go life and death problems using brute force search. To achieve this, certain key requirements needs to be met:
