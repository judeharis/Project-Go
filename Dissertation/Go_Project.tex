...
\begin{enumerate}
  ...
  \item Each turn consists of few parts as follows:
  	\begin{enumerate}[label={(\alph*)}]
		...
		\item Then any of the opponent’s stones that does not have a liberty is removed from the board. See example 1.
		\item A move is invalid if it will cause one or more stone of the current player’s colour to be captured – this is to prevent self-capturing moves. See example 2.
		\item Removal of opponent’s stones take precedence over self-capture check. This allows for moves which captures opponent’s stones but is suicide if the opponent’s stones are not removed first. See example 3.
	\end{enumerate}
  \item Ko rule – Player is not allowed to place a stone on point A if it captures one stone which was placed on point B during the opponent’s previous turn which captured one stone on point A. This is to prevent infinite repetition of the same few moves. See example 4.
...
\noindent Liberty -  Liberty of a stone is any adjacent empty intersection of the stone or any empty intersection of any stone connected to the original stone. See example 5.

\noindent Connected - Two stones are connected if they are of the same colour and is adjacent to each other or there is a set of stones of the same colour which are connected to both stones. See example 6.

\noindent String – Set of stones that are connected.
