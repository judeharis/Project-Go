...
The Go board is represent as an object class within GoLD which contains all the vital data to represent the current board position. GoLD's board is desgined for the purpose of making tree search and board evaluations as quick as possible. This meant storing more imformation within the board object was ideal  instead of recalaculating information every time it was needed. The board object also implements public methods which allows the computer and also the users to interact with the board. The board in essence implements the rules of Go and automatically updates the board position once a move is played either by the user or the computer.


\subsection{Stones}
Representation

\subsection{Vaild Move Generator}
