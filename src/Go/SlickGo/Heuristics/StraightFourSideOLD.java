...
public class StraightFourSideOLD  {
	Evaluator e;
	PatternSearcher ps;

	public StraightFourSideOLD (Evaluator e){
		this.e=e;
	}
...
//		ArrayList<ArrayList<Tuple>> pMatches =ps.allStringMatchv2(sstring, pattern);
		...
		ArrayList<Pattern> pattern = Pattern.sToPv2("xrxrxrxrxrxdxzdxdS");
		...
		if(!pMatches.isEmpty()) {
			...
			for(ArrayList<Tuple> tlist: pMatches) {
				if(!tlist.isEmpty()) {
					...
					if(e.isThere(S2) || e.isThere(S5)) continue;
					
					retval += 1000;
					if (e.isTheres(S3,S4))retval += 500;
					else if (e.isThere(S3) || e.isThere(S4))retval += 500;
