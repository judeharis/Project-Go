...
public class Editor extends BasicGameState {
	...
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
	    ...
	    font =  new TrueTypeFont(new java.awt.Font(java.awt.Font.SERIF,java.awt.Font.PLAIN , 18), false);
	    desc = new TextField(gc, font, board.boardSize , board.TileSize +460, 300, 60); 
	    desc.setTextColor(Color.red);
	    desc.setBorderColor(Color.black);
	}
	...
	public void update(GameContainer gc, StateBasedGame sbg, int delta) throws SlickException {
		...
		if (input.isMousePressed(0)) {
			if (SlickGo.withinBounds(bx,by)) if (!board.computer) board.takeTurn(bx,by , true);
		

			
			if (SlickGo.regionChecker(board.boardSize ,board.TileSize +40,200,40,gc)) {board.placing = Stones.BLACK;board.tempkeystone=Stones.KEYBLACKSTONE;}
			
			if (SlickGo.regionChecker(board.boardSize ,board.TileSize +80,200,40,gc)) {board.placing = Stones.WHITE;board.tempkeystone=Stones.KEYWHITESTONE;}
			...
			if (SlickGo.regionChecker(board.boardSize ,board.TileSize +400,200,40,gc)){board.keystone = board.tempkeystone;board.placing = board.keystone;}
